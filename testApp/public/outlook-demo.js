...
//var appId = '91bd379f-d2a8-4ee3-88eb-6a0ff4506dc9'; davids azure ID
...
$(function() {
    ...
    var appId = '7ef5491d-6e65-47d0-adf1-6c25997c9c74';
    var scopes = 'openid profile User.Read Mail.Read Contacts.Read';
    var oDataResults = '';
    var apiURL = '';
    var firstPage = true;
    hideMainFunctions();
    //const searchFunction = require('./searchFunctions');
    ...
    function render(hash) {
      ...
      var pagemap = {
        ...
        '#inbox': function () {
          if (isAuthenticated) {
            //this is the first time renderInbox is called
            renderInbox(false);
            //displaySearchBox();--------------------------------------------------------
          } else {
            // Redirect to home page
    ...
    function hideMainFunctions() {
      const hideMe = document.getElementById('showOrHide');
      hideMe.style.display = 'none';
    }

    function showMainFunctions() {
      const showMe =document.getElementById('showOrHide');
      showMe.style.display = 'block';
    }

    function renderInbox(buttonType) {
      ...
      getUserInboxMessages(function(messages, error){
        ...
        if (error) {
          ...
          $('#inbox-status').text('This is where your Search Results will appear from your inbox.');
    ...
    function getUserInboxMessages(callback, buttonType) {
      ...
      getAccessToken(function(accessToken) {
        if (accessToken) {
          ...
          // check API call
            if(buttonType === false) {
              apiURL = '/me/mailfolders/inbox/messages';
            } else if(buttonType === 'next') {
              apiURL = oDataResults['@odata.nextLink'];
            } else {
              var newArr = apiURL.split('skip=');
              var newSkipInt = parseInt(newArr[1]) - 100;
              if(newSkipInt === 0) {
                apiURL = '/me/mailfolders/inbox/messages';
              } else {
                apiURL = newArr[0] + 'skip=' + newSkipInt.toString();
              } 
            }
            
          // Get the 10 newest messages / make the call
          client
            .api(apiURL)
            .top(100)
            ...
            .get((err, res) => {
              ...
              // add pagination...
                ...
                if(oDataResults.hasOwnProperty('@odata.nextLink') != undefined && oDataResults.hasOwnProperty('@odata.nextLink') === true) {
                  const term = document.getElementById('search').value.toLowerCase();
                  filterSearch(term);
                  excludeFilterSearch();
                    //show next button
                    document.getElementById('next').style.display = 'block';
                    if (oDataResults['@odata.nextLink'].indexOf('skip=100') > -1) {
                      filterSearch(term);
                      excludeFilterSearch();
